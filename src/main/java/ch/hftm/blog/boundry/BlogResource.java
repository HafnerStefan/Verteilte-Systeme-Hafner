...
public class BlogResource {
	...
	public Response fetchAllBlogs(@QueryParam("userId") Long userId, @QueryParam("start") @DefaultValue("0") int start, @QueryParam("size") @DefaultValue("15") int size) {
		...
		if (userId != null && blogListDTO.isEmpty()) {
			...
			throw new ObjectIsEmptyException("Return list of Blogs is Empty: the user: " + userId + " has max " + sizeMax + " blogs. Do you want see blog " + (start * size) + " to " + (start * size + size));
		}
